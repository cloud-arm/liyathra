...
?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      ...
      <div class="row" id="util_add" style="display: none;">
          <div class="col-md-6">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title" style="width: 100%;">New Utility Type
                      <span class="btn p-0 me-2  pull-right "> <i onclick="util_cl()"
                        class="fa-solid fa-xmark" style="font-size: 25px;"></i> </span>
                      </h3>
                </div>
      ...
      <div class="box box-info">
        ...
        <div class="box-body d-block">
            ...
            <div class="col-md-3" >
              ...
              <div class="col-md-4" id="job_sec" style="display: none;">
                <div class="form-group">
                  <label>Jobs</label>
                  <select class="form-control select2" name="job_no" style="width: 100%;" tabindex="7" >
                    <option value="0"></option>
                  <?php
                  $result = $db->prepare("SELECT * FROM job WHERE type='check_in' OR type='Out' ");
                  $result->bindParam(':userid', $ttr);
                  $result->execute();
                  for($i=0; $row = $result->fetch(); $i++){
                    ?>
                      <option value="<?php echo $id=$row['id'];?>" > <?php echo $id.' - '.$row['vehicle_no'].' - '.$row['invoice_no']; ?> </option>
                    <?php
                          }
                        ?>
                  </select>
              ...
              <div class="col-md-3 util_sec" style="display: none;">
                <div class="form-group">
            ...
            <table id="example" class="table table-bordered table-striped" style="border-radius: 0;">
              ...
              <tbody>
                ...
                <tr class="record" style="<?php echo $style; ?>" >			
                  ...
                  <td><?php if($row['util_id']>0){ echo $row['util_name']; }else{ echo $row['type']; }  ?> </td>
                  <td><?php echo $row['comment'];   ?></td>
                  <td><?php echo $row['pay_type'];   ?></td>
                  <td><?php echo $row['chq_no'];   ?></td>
                  <td><?php echo $row['chq_date'];   ?></td>
                  <td>Rs.<?php echo $row['amount'];$tot+=$row['amount'];  ?></td>
...
<?php include("script.php");?>
 ...
 <script type="text/javascript">
  ...
  function select_pay(){
    ...
    if(val == 'cash'){$('#acc_sec').css('display', 'block');}else{$('#acc_sec').css('display', 'none');}
    if(val == 'chq'){$('.bank_sec').css('display', 'block');}else{$('.bank_sec').css('display', 'none');}
  ...
  function select_type(){
    ...
    if(val == 3){$('#job_sec').css('display', 'block');$('.util_sec').css('display', 'none');
    }else
    if(val == 4){$('.util_sec').css('display', 'block');$('#job_sec').css('display', 'none');
    }else{$('#job_sec').css('display', 'none');$('.util_sec').css('display', 'none');}
  }
...
$(this).parents(".record").css({'opacity': '0.5', 'cursor': 'default'})
$(this).remove();
...
<script>
  $(function () {
    ...
    $('#datepicker_set').datepicker({  autoclose: true, datepicker: true,  format: 'yyyy-mm-dd '});
