...
$result = $db->prepare("SELECT * FROM inquiries WHERE invoice_no= :id");
...
if ($job_type == 0) {
	...
	$sql = 'UPDATE inquiries SET job_type =?, type_name =? WHERE id = ? ';
	$ql = $db->prepare($sql);
...
$q = $db->prepare($sql);
$q->execute(array($invo, $pro_id, $qty, $price, $name, $price, $profit, $code, $discount, $date, $emp, $sup_emp, 'Inquiries', $job_no));
