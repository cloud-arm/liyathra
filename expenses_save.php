...
if($unit == 1){
    ...
    $re->execute();
    for($i=0; $r = $re->fetch(); $i++){$type_name=$r['type_name'];}
    ...
    if($type_name == 'Utility_Bill'){
        ...
        $re->execute();
        for($i=0; $r = $re->fetch(); $i++){$util_name=$r['name']; }
    }
    ...
    if($pay_type == 'chq'){
        ...
        $re->execute();
        for($k=0; $r = $re->fetch(); $k++){$blc=$r['amount']; $acc_name=$r['name'];}
    ...
    if($type_name == 'Utility_Bill'){
        ...
        $re->bindParam(':id', $util_id);
        $re->execute();
        for($k=0; $r = $re->fetch(); $k++){$util_fw_blc=$r['credit'];}
    }
    ...
    if($pay_type == 'cash'){
        ...
        $ql = $db->prepare($sql);
        $ql->execute(array('Expenses','Debit',$type,$amount,0,$acc,'Cash',$acc_name,$cr_blc,'Cash Payment',$type_name,$type,0,$date,$time,$ui,$un));
    }
